...
namespace Graph {
  ...
  std::optional<typename Router<Weight>::RouteInfo> Router<Weight>::BuildRoute(VertexId from, VertexId to) const {
    ...
    while (!vertices_by_weight.empty()) {
      ...
      const auto current_vertex = min_vertex_it->second;
      const auto current_weight = min_vertex_it->first;
      vertices_by_weight.erase(min_vertex_it); 
      done_vertices.insert(current_vertex);
      for (const EdgeId edge_id : graph_.GetIncidentEdges(current_vertex)) {
        const auto& edge = graph_.GetEdge(edge_id);
        if (done_vertices.count(edge.to)) {
          continue; 
        }
        if (!routes_internal_data_[edge.to] || routes_internal_data_[edge.to]->weight > current_weight + edge.weight) {
          if (routes_internal_data_[edge.to]) {
            vertices_by_weight.erase({routes_internal_data_[edge.to]->weight, edge.to});
          }
          routes_internal_data_[edge.to] = RouteInternalData{.weight = current_weight + edge.weight,
                                                             .prev_edge = edge_id}; 
