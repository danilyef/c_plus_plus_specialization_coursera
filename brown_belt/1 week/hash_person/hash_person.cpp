...
struct AddressHasher {
  size_t operator() (const Address& a) const {
    size_t h1 = shash(a.city);
    size_t h2 = shash(a.street);
    size_t h3 = a.building;
    size_t x = 2053;
...
struct PersonHasher {
  size_t operator() (const Person& p) const {
    size_t h1 = shash(p.name);
    size_t h2 = p.height;
    size_t h3 = dhash(p.weight);
    size_t h4 = ashash(p.address);
    size_t x = 2053;

    return  h1 * x * x * x + h2 * x * x + h3 * x + h4;
    
  }
  hash<string> shash;
  hash<double> dhash;
  AddressHasher ashash;
};
