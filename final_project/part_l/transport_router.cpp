...
/*Setters*/
void TransportRouter::SetRoutingSettings(Json::Node routing_settings_node){
    routing_settings.bus_wait_time = routing_settings_node.AsMap().at("bus_wait_time").AsDouble() * 1.0;
    routing_settings.bus_velocity = routing_settings_node.AsMap().at("bus_velocity").AsDouble() * 1000.0;
};
