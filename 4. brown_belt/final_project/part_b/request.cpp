...
void AddStopRequest::ParseFromString(std::string_view input){
    ...
    while (!input.empty()){
        double distance = ConvertToDouble(ReadToken(input, "m to "));
        std::string stop_name = std::string(ReadToken(input, ", "));
        neighbors[std::move(stop_name)] = distance;
    }
}
...
std::ostream& operator<<(std::ostream& os, const BusInformation& bus_info){
    if(bus_info.found){
        os << bus_info.stops << " stops on route, " << bus_info.unique_stops << " unique stops, " << bus_info.route_len << " route length, " << bus_info.curvature << " curvature";
    } else {
        os << "not found";
    }
    return os;
